...
function isValidPathItem(item) {
    return item && item.length > 0 && item !== '.' && item !== '..';
}
...
Context.prototype._dispatch = function(req, res, callback) {
    ...
    var keys = url.parse(req.url).pathname.split('/').filter(isValidPathItem);

    // Forbid direct access to system items.
    for (var i = 0, M = keys.length; i < M; ++i) {
        if (isSystemItem(keys[i])) {
            callback(null);
            return;
        }
    }

    // FIXME: compose real path.
    keys.unshift('./htdocs');
    var realPath = keys.join('/');

    // TODO: cascade options of each folders.
    var options = {};

    // FIXME: cache fs stats results.
    fs.stat(realPath, function(err, stats){
        if (!stats) {
            // TODO: use consider cascaded options.
            callback(null);
        } else if (stats.isDirectory()) {
            callback({path: realPath + '/_folder', options: options});
        } else {
            callback({path: realPath, options: options});
        }
    });
}
